...
const StyledButton = styled.button<
  Pick<
    IconButtonProps,
    'variant' | 'size' | 'position' | 'accent' | 'focus' | 'color'
  >
>`
  align-items: center;
  ${({ theme, variant, accent, disabled, focus, color }) => {
    if (color) {
      return css`
...
export type SIconButtonProps = Omit<
  IconButtonProps,
  'variant' | 'size' | 'position' | 'accent' | 'focus' | 'color'
>;

export const IconButton = ({ disabled, ...props }: IconButtonProps) => {
  if (disabled === true) {
    return null;
  }
  ...
  return (
    <StyledButton
      ...
      color={props.color}
    >
      {props.Icon && <props.Icon size={theme.icon.size.md} />}
    </StyledButton>
  );
};

...
export const IconPicker = ({
  ...
  const matchingSearchIconKeys = useMemo(() => {
    ...
    const scoredIcons = Object.keys(icons)
      .filter((key) => !COLORED_ICONS[key as keyof typeof COLORED_ICONS])
      .map((iconKey) => ({
        iconKey,
        score: scoreIconMatch(iconKey, searchString),
      }));

    const scoredColoredIcons = Object.keys(COLORED_ICONS).map((iconKey) => ({
      iconKey,
      score: scoreIconMatch(iconKey, searchString),
      colored: true,
    }));

    const filteredAndSortedIconKeys = [
      ...scoredColoredIcons
  ...
  const iconKeys2d = useMemo(
    () => arrayToChunks(matchingSearchIconKeys, 5),
          ...
          <IconButton
            ...
            color={
              selectedIconKey
                ? theme.color[
                    COLORED_ICONS[
                      selectedIconKey as keyof typeof COLORED_ICONS
                    ] || ''
                  ]
                : undefined
            }
              ...
              >
                <DropdownMenuItemsContainer>
                  <StyledMenuIconItemsContainer>
                    {iconKeys2d.map((iconKeyRow, rowIndex) => (
                      <React.Fragment key={`icon-row-${rowIndex}`}>
                        {iconKeyRow.map((iconKey) => {
                          const Icon = getIcon(iconKey);
                          const iconColor =
                            COLORED_ICONS[
                              iconKey as keyof typeof COLORED_ICONS
                            ];
                          return (
                            <IconPickerIcon
                              key={iconKey}
                              ...
                              color={
                                iconColor ? theme.color[iconColor] : undefined
                              }
                            />
                          );
                        })}
